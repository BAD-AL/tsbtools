...
namespace TSBTool2
{
    ...
    public class InputParser
    {
        ...
        public void ReadFromStdin()
        {
            ...
            {
                TSBTool.StaticUtils.ShowError(string.Format(
                 "Error Processing line {0}:'{1}'.\n{2}\n{3}",
        ...
        public static String CheckTextForRedundentSetCommands(String input)
        {
            ...
            {
                current = mc[i];
                location1 = TSBTool.StaticUtils.ParseLongFromHexString(current.Groups[1].ToString().Substring(2)); // long.Parse(current.Groups[1].ToString().Substring(2), System.Globalization.NumberStyles.AllowHexSpecifier);
                valueLength1 = current.Groups[2].Length / 2;
                for (int j = i + 1; j < mc.Count; j++)
                {
                    m = mc[j];
                    location2 = TSBTool.StaticUtils.ParseLongFromHexString(m.Groups[1].ToString().Substring(2)); // long.Parse(m.Groups[1].ToString().Substring(2), System.Globalization.NumberStyles.AllowHexSpecifier);
        ...
        protected virtual void ProcessLine(string line)
        {
            ...
            /*else if( (m = juiceRegex.Match(line)) != Match.Empty )
            {
                ...
                else 
                {
                    ...
                    {
                        TSBTool.StaticUtils.AddError(string.Format("ERROR! Line = '{0}'",line));
                    }
                }
            }*/
            else if (line.StartsWith("ReplaceString"))
            {
                ...
                if (repMatch.Groups.Count > 1)
                {
                    ...
                    String msg = TSBTool.StaticUtils.ReplaceStringInRom(tool.OutputRom, find, replace, occur);
                ...
                {
                    TSBTool.StaticUtils.AddError(String.Format("ERROR! Not enough info to use 'ReplaceString' function.Line={0}", line));
                }
            }
            ...
            else if (teamRegex.Match(line) != Match.Empty)//line.StartsWith("TEAM") )
            {
                ...
                if (!ret)
                {
                    ...
                    TSBTool.StaticUtils.AddError(string.Format("Team input must be in the form 'TEAM = team SimData=0x1F'"));
                    return;
                }
                HandleSimData(line);
            ...
            {
                TSBTool.StaticUtils.AddError(string.Format("Garbage/orphin line not applied \"{0}\"", line));
            }
        }
        ...
        private void SetYear(string line)
        {
            ...
            if (year.Length < 1)
            {
                TSBTool.StaticUtils.AddError(string.Format("'{0}' is not valid.", line));
            }
            else
            {
                tool.SetYear(year);
        ...
        public static int[] GetSimData(string line)
        {
            ...
            if (data.Length > 0)
            {
                try
                {
                    int simData = TSBTool.StaticUtils.ParseIntFromHexString(data);
                    ret[0] = simData;
                }
                catch
                {
                    TSBTool.StaticUtils.AddError(string.Format("Error getting SimData with line '{0}'.", line));
                }
            }
        ...
        private int GetWeek(string line)
        {
            ...
            {
                TSBTool.StaticUtils.AddError(string.Format("Week '{0}' is invalid.", week_str));
            }
            return ret;
        }
        ...
        private bool SetCurrentTeam(string team)
        {
            ...
            {//error condition
                TSBTool.StaticUtils.AddError(string.Format("Team '{0}' is Invalid.", team));
                return false;
            }
            else
        ...
        protected virtual void UpdateRoster(string line)
        {
            ...
            else
            {
                ...
                if (line.IndexOf("#") > -1)
                {
                    ...
                    {
                        TSBTool.StaticUtils.AddError(string.Format("ERROR! (jersey number) Line  {0}", line));
                        return;
                    }
                }
                ...
                {
                    TSBTool.StaticUtils.AddError(string.Format("ERROR! With line \"{0}\"     team = {1}", line, currentTeam));
                }
            }
        }
        ...
        private void SetQB(string line)
        {
            ...
            if (jerseyNumber < 0)
            {
                TSBTool.StaticUtils.AddError(string.Format("Error with jersey number for '{0} {1}', setting to 0.", fname, lname));
                jerseyNumber = 0;
            }
            tool.InsertPlayerName(season, currentTeam, pos, fname, lname, (byte)jerseyNumber);

            byte[] vals = TSBTool.StaticUtils.GetTsbAbilities(GetInts(line,false));
        ...
        protected virtual void SetDefensivePlayer(string line)
        {
            ...
            if (vals != null && vals.Length > 5)
                ...
                TSBTool.StaticUtils.AddError(string.Format("Warning! On line '{0}'. Invalid player attributes.", line));
            ...
            if (input != null)
            {
                ...
                try
                {
                    for (i = 0; i < nums.Length; i++)
                    {
                        ...
                        {
                            if (useHex)
                                result[j++] = TSBTool.StaticUtils.ParseIntFromHexString(s);
                            else
        ...
        public static  int GetFace(string line)
        {
            ...
            if (m != Match.Empty)
            {
                ...
                {
                    ret = -1;
                    TSBTool.StaticUtils.AddError(string.Format("Face ERROR line '{0}'", line));
                }
            }
        ...
        public static  string GetLastName(string line)
        {
            ...
            if (m != Match.Empty)
            {
                ...
                if (m2 != Match.Empty)
                    ...
                    TSBTool.StaticUtils.WriteError("ERROR Getting last name for>" + line);
            }
            return ret;
        }
        ...
        public static  string GetFirstName(string line)
        {
            ...
            if (m != Match.Empty)
            {
                ...
                if (m2 != Match.Empty)
                    ...
                    TSBTool.StaticUtils.WriteError("ERROR Getting first name for>" + line);
            }
            return ret;
        }*/
        ...
        public static byte[] GetBytesFromString(string byteString)
        {
            ...
            if (byteString != null && byteString.Length > 1 && (byteString.Length % 2) == 0)
            {
                ...
                for (int i = 0; i < tmp.Length; i++)
                {
                    ...
                    tmp[i] = TSBTool.StaticUtils.ParseByteFromHexString(b); // byte.Parse(b, System.Globalization.NumberStyles.AllowHexSpecifier);
                }
                ret = tmp;
            }
            return ret;
