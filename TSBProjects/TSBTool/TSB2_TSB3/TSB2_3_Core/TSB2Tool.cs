...
namespace TSBTool2
{
    ...
    // TODO:
    // 4. NFL Records ?
    public class TSB2Tool : ITecmoTool, TSBTool.ITecmoContent
    {
        public static bool IsTecmoSuperBowl2Rom(byte[] rom)
        {
            ...
            {
                List<long> results = TSBTool.StaticUtils.FindStringInFile("CONSECUTIVE", rom, 0x1be4e0, 0x1bef20);
                if (results.Count > 0)
                {
                    retVal = true;
                }
        ...
        public bool ShowOffPref { get; set; }
        ...
        public virtual TSBTool.ROM_TYPE RomVersion { get { return TSBTool.ROM_TYPE.SNES_TSB2; } }
            ...
            string team, string position)
        {
            ...
            byte ps = TSBTool.StaticUtils.GetFirstNibble(OutputRom[location + 3]);
            byte pc = TSBTool.StaticUtils.GetSecondNibble(OutputRom[location + 3]);
            byte pa = TSBTool.StaticUtils.GetFirstNibble(OutputRom[location + 4]);
            byte ar = TSBTool.StaticUtils.GetSecondNibble(OutputRom[location + 4]);
            byte co = TSBTool.StaticUtils.GetSecondNibble(OutputRom[location + 5]);
            byte bb = TSBTool.StaticUtils.GetFirstNibble(OutputRom[location + 6]);
            byte sp = TSBTool.StaticUtils.GetSecondNibble(OutputRom[location + 6]); // sim pocket??
            ...
            //byte coolness)
        {
            ...
            byte ps_pc = TSBTool.StaticUtils.CombineNibbles(abilities[5], abilities[6]);
            byte pa_ar = TSBTool.StaticUtils.CombineNibbles(abilities[7], abilities[8]);
            byte unk1 = TSBTool.StaticUtils.GetFirstNibble(OutputRom[location + 5]);
            byte unk1_co = TSBTool.StaticUtils.CombineNibbles(unk1, abilities[9]);
            byte unk2 = TSBTool.StaticUtils.GetSecondNibble(OutputRom[location + 6]);
            byte bb_unk2 = TSBTool.StaticUtils.CombineNibbles(abilities[4], unk2);
            ...
            string team, string position)
        {
            ...
            byte ms = TSBTool.StaticUtils.GetFirstNibble(OutputRom[location + 1]);
            byte hp = TSBTool.StaticUtils.GetSecondNibble(OutputRom[location + 1]);
            byte bb = TSBTool.StaticUtils.GetFirstNibble(OutputRom[location + 3]);
            ...
            //byte bodyBalance)
        {
            ...
            byte rs_rp = TSBTool.StaticUtils.CombineNibbles(abilities[0], abilities[1]);
            byte ms_hp = TSBTool.StaticUtils.CombineNibbles(abilities[2], abilities[3]);
            byte unk1 = TSBTool.StaticUtils.GetSecondNibble(OutputRom[location + 3]);
            byte bb_unk1 = TSBTool.StaticUtils.CombineNibbles(abilities[4], unk1);
            SetByte(location, rs_rp);
            SetByte(location + 1, ms_hp);
            ...
            string team, string position)
        {
            ...
            byte ms = TSBTool.StaticUtils.GetFirstNibble(OutputRom[location + 1]);
            byte hp = TSBTool.StaticUtils.GetSecondNibble(OutputRom[location + 1]);
            byte kp = TSBTool.StaticUtils.GetSecondNibble(OutputRom[location + 3]);
            byte ka = TSBTool.StaticUtils.GetFirstNibble(OutputRom[location + 4]);
            byte ab = TSBTool.StaticUtils.GetSecondNibble(OutputRom[location + 4]);
            byte bb = TSBTool.StaticUtils.GetFirstNibble(OutputRom[location + 5]);
            ...
            //byte kickingAccuracy, byte avoidBlock)
        {
            ...
            byte rs_rp = TSBTool.StaticUtils.CombineNibbles(abilities[0], abilities[1]);
            byte ms_hp = TSBTool.StaticUtils.CombineNibbles(abilities[2], abilities[3]);
            byte unk1 = TSBTool.StaticUtils.GetFirstNibble(OutputRom[location + 3]);
            byte unk1_kp = TSBTool.StaticUtils.CombineNibbles(unk1, abilities[5]);
            byte ka_ab = TSBTool.StaticUtils.CombineNibbles(abilities[6], abilities[7]);
            byte unk2 = TSBTool.StaticUtils.GetSecondNibble(OutputRom[location + 5]);
            byte bb_unk2 = TSBTool.StaticUtils.CombineNibbles(abilities[4], unk2);
            ...
            string team, string position)
        {
            ...
            byte ms = TSBTool.StaticUtils.GetFirstNibble(OutputRom[location + 1]);
            byte hp = TSBTool.StaticUtils.GetSecondNibble(OutputRom[location + 1]);
            byte kp = TSBTool.StaticUtils.GetFirstNibble(OutputRom[location + 3]);
            byte ab = TSBTool.StaticUtils.GetSecondNibble(OutputRom[location + 3]);
            byte bb = TSBTool.StaticUtils.GetFirstNibble(OutputRom[location + 4]);
            ...
            //byte kickingPower, byte avoidBlock)
        {
            ...
            byte rs_rp = TSBTool.StaticUtils.CombineNibbles(abilities[0], abilities[1]);
            byte ms_hp = TSBTool.StaticUtils.CombineNibbles(abilities[2], abilities[3]);
            byte kp_ab = TSBTool.StaticUtils.CombineNibbles(abilities[5], abilities[6]);
            byte unk1 = TSBTool.StaticUtils.GetSecondNibble(OutputRom[location + 4]);
            byte bb_unk1 = TSBTool.StaticUtils.CombineNibbles(abilities[4], unk1);
        ...
        protected virtual string GetSkill_DefPlayerAbilities(int season, string team, string position)
        {
            ...
            byte bc = TSBTool.StaticUtils.GetFirstNibble(OutputRom[location + 3]);
            byte rec = TSBTool.StaticUtils.GetSecondNibble(OutputRom[location + 3]);
            byte bb = TSBTool.StaticUtils.GetFirstNibble(OutputRom[location + 4]);
            ...
            //byte ballControl, byte receptions)
        {
            ...
            byte rs_rp = TSBTool.StaticUtils.CombineNibbles(abilities[0], abilities[1]);
            byte ms_hp = TSBTool.StaticUtils.CombineNibbles(abilities[2], abilities[3]);
            byte bc_rec = TSBTool.StaticUtils.CombineNibbles(abilities[5], abilities[6]);
            byte unk1 = TSBTool.StaticUtils.GetSecondNibble(OutputRom[location + 4]);
            byte bb_unk1 = TSBTool.StaticUtils.CombineNibbles(abilities[4], unk1);
            SetByte(location, rs_rp);
            SetByte(location + 1, ms_hp);
            ...
            //byte interceptions, byte quickness)
        {
            ...
            byte rs_rp = TSBTool.StaticUtils.CombineNibbles(abilities[0], abilities[1]);
            byte ms_hp = TSBTool.StaticUtils.CombineNibbles(abilities[2], abilities[3]);
            byte pi_qu = TSBTool.StaticUtils.CombineNibbles(abilities[5], abilities[6]);
            byte unk1 = TSBTool.StaticUtils.GetSecondNibble(OutputRom[location + 4]);
            byte bb_unk1 = TSBTool.StaticUtils.CombineNibbles(abilities[4], unk1);
            SetByte(location, rs_rp);
            SetByte(location + 1, ms_hp);
        ...
        public virtual void SetFace(int season, string team, string position, int face)
        {
            ...
            int location = GetPlayerAttributeLocation(season, team, position) + 2;
            byte lowNibble = TSBTool.StaticUtils.GetSecondNibble(OutputRom[location]);
            byte highNibble = TSBTool.StaticUtils.GetFirstNibble((byte)face);
            byte face_b = TSBTool.StaticUtils.CombineNibbles(highNibble, lowNibble);
        ...
        public string GetPlayerName(int season, string team, string position, out byte playerNumber)
        {
            ...
            string name = TSBTool.StaticUtils.GetStringTableString(OutputRom, playerIndex, first_ptr, offset);
        ...
        public void InsertPlayerName(int season, string currentTeam, string position, string fname, string lname, byte jerseyNumber)
        {
            ...
            TSBTool.StaticUtils.SetStringTableString(OutputRom, playerIndex, insertThis, first_ptr, offset, stringsInTable, NAME_TABLE_SIZE);
        }
        ...
        {
            TSBTool.StaticUtils.CheckTSB2Args(season, team);
            byte[] theBytes = TSBTool.StaticUtils.GetHexBytes(data);
            int location = GetSimLocation(season, team, "K") + 1;
        ...
        {
            string retVal = TSBTool.StaticUtils.GetStringTableString(OutputRom, teamIndex, tsb2_team_name_string_table_first_ptr,
        ...
        {
            string retVal = TSBTool.StaticUtils.GetStringTableString(OutputRom, teamIndex, tsb2_team_name_string_table_first_ptr, tsb2_team_name_string_table_offset).Substring(5);
            int lastSpace = retVal.LastIndexOf(' ');
            if (lastSpace > -1)
        ...
        {
            string retVal = TSBTool.StaticUtils.GetStringTableString(OutputRom, teamIndex, tsb2_team_name_string_table_first_ptr, tsb2_team_name_string_table_offset);
        ...
        public virtual void SetTeamAbbreviation(int teamIndex, string abb)
        {
            if (abb != null && abb.Length > 0)
            {
                ...
                TSBTool.StaticUtils.SetStringTableString(OutputRom, teamIndex, teamString, tsb2_team_name_string_table_first_ptr, tsb2_team_name_string_table_offset, 30, TEAM_NAME_STRING_TABLE_SIZE);
            }
        }
        ...
        public void SetPlaybook(int season, string team, string runs, string passes)
        {
            ...
            byte[] runBytes = TSBTool.StaticUtils.GetHexBytes(runs.Substring(1));
            byte[] passBytes = TSBTool.StaticUtils.GetHexBytes(passes.Substring(1));
            // playbook 1
            if (runBytes.Length > 1 && passBytes.Length > 1)
            {
                ...
                TSBTool.StaticUtils.AddError(string.Format("ERROR Settings playbook for season:{0} team:{1} data={2} {3}",
...
", this.RomVersion);
        ...
        {
            TSBTool.StaticUtils.SaveRom(fileName, OutputRom);
        }
    }
}
