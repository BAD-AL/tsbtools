...
namespace TSBTool
{
    ...
    public class TecmoTool : ITecmoTool, ITecmoContent
	{
        ...
        {
            bool ret = false;
            if (len == TecmoToolFactory.ORIG_NES_TSB1_LEN)
                ret = true;
            return ret;
        }
...
Do you want to continue?", "<file>", len, TecmoToolFactory.ORIG_NES_TSB1_LEN, TecmoToolFactory.CXROM_V105_LEN, TecmoToolFactory.CXROM_V111_LEN);
                        result = StaticUtils.ShowConfirmationDialog("WARNING!", message);
					}
					else
					{
    ...
    If you know this is really a nes TSB ROM, you can force TSBToolSupreme to load it in GUI mode.",
			...
			byte number)
		{
			...
			else
			{
				...
				{
					StaticUtils.ShowError(string.Format("Error on name {0} {1}", fname, lname));
					return;
				}
				...
				{
					StaticUtils.AddError(string.Format("Warning!! There is a 15 character limit for names\n '{0} {1}' is {2} characters long.",
...
//					));
		...
		public virtual void SetTeamName(int teamIndex, string name)
		{
            if(name.Length > 1)
                ...
                StaticUtils.AddError("'SetTeamCity': team name must not be empty");
		}
		...
		public virtual void SetTeamCity(int teamIndex, string city)
		{
            if (city.Length > 1)
                ...
                StaticUtils.AddError("'SetTeamCity': city name must not be empty");
		}
		...
		protected  byte[] m_JuiceArray = 
	   {
		...
		public virtual void SetHomeUniform(string team, string colorString)
		{
			...
			{
				StaticUtils.AddError(string.Format("ERROR setting Uniform1 for team {0},'{1}'",
					team,colorString));
			}
		}
		...
		public virtual void SetAwayUniform(string team, string colorString)
		{
			...
			{
				StaticUtils.AddError(string.Format("ERROR setting Uniform2 for team {0},'{1}'",
					team,colorString));
			}
		}
