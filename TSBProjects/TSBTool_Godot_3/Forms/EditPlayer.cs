...
public partial class EditPlayer : Control
{
	...
	public override void _Ready()
	{
		...
		// signals hook up
		m_PrevPicture.Connect("pressed",this, nameof( OnPevPicturePressed));
		m_NextPicture.Connect("pressed",this, nameof(OnNextPicturePressed));
		closeButton.Connect  ("pressed",this, nameof(OnCloseButtonPressed));
		m_PositionComboBox.Connect("item_selected", this, nameof(_on_m_position_combo_box_item_selected));
		m_TeamsComboBox.Connect("item_selected",this, nameof(_on_m_teams_combo_box_item_selected));
		m_FirstNameTextBox.Connect("focus_exited",this, nameof(m_FirstNameTextBox_Leave));
		m_LastNameTextBox.Connect( "focus_exited",this, nameof(m_LastNameTextBox_Leave));
		m_FaceBox.Connect("TextureRectClicked", this, nameof(FaceBoxClicked));
	...
	private void Value_Changed(double val)
	{
		if (!changingPlayer)
		{
			GD.Print($"EditPlayer.ValueChanged {val}");
			ReplacePlayer();
		}
	}
	...
	private void FaceBoxClicked(Vector2 mousePos)
	{
	...
	private void ShowFacePicker()
	{
		...
		facePickerDialog = (FacePickerDialog)facePickerScene.Instance();
	...
	private void facePicker_Closed()
	{
		...
		{
			GD.Print($"facePicker_Closed(), item = {facePickerDialog.SelectedItem}");
	...
	{
		GD.Print("EditPlayer.OnPevPicturePressed()");
	...
	{
		GD.Print("EditPlayer.OnNextPicturePressed()");
	...
	private void ReplacePlayer(string team, string oldPlayer, string newPlayer)
	{
		...
		if (test != m_TeamsComboBox.GetItemCount() - 1)
		{
...
Please verify that this player's attributes are correct.", oldPlayer);
	...
	private System.Collections.Generic.Dictionary<string, ImageInfo> _imageData;
	/// <summary>
	...
	private void ShowCurrentFace()
	{
		if(_imageData == null)
		{
			...
			string filePath = "TSBTool_Godot_3.faces.png.json";
			string data = ReadEmbeddedResource(filePath);
			_imageData = ImageInfoCollection.CreateAsDictionary(ImageInfoCollection.FromJsonString(data));
			GD.Print($"ImageData initialized, images count : {_imageData.Count}");
		}
		string file = string.Format("{0:x2}.BMP", m_ImageNumber).ToUpper();
		GD.Print($"Looking for file:{file}");
		if(_imageData != null && _imageData.ContainsKey(file))
		{
			ImageInfo imageInfo = _imageData[file];
			faceAtlasTexture.Region = new Rect2( imageInfo.x, imageInfo.y, imageInfo.width, imageInfo.height);
